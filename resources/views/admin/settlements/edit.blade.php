    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.settlements.update", [$settlement->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id" required>
                    @foreach($companies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $settlement->company->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('transactions') ? 'is-invalid' : '' }}" name="transactions[]" id="transactions" multiple required>
                    @foreach($transactions as $id => $transaction)
                        <option value="{{ $id }}" {{ (in_array($id, old('transactions', [])) || $settlement->transactions->contains($id)) ? 'selected' : '' }}>{{ $transaction }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_pabl_amount') ? 'is-invalid' : '' }}" type="text" name="total_pabl_amount" id="total_pabl_amount" value="{{ old('total_pabl_amount', $settlement->total_pabl_amount) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_paid_amount') ? 'is-invalid' : '' }}" type="text" name="total_paid_amount" id="total_paid_amount" value="{{ old('total_paid_amount', $settlement->total_paid_amount) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paid_by') ? 'is-invalid' : '' }}" type="text" name="paid_by" id="paid_by" value="{{ old('paid_by', $settlement->paid_by) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_medium') ? 'is-invalid' : '' }}" type="text" name="payment_medium" id="payment_medium" value="{{ old('payment_medium', $settlement->payment_medium) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('payment_ref') ? 'is-invalid' : '' }}" name="payment_ref" id="payment_ref">{{ old('payment_ref', $settlement->payment_ref) }}</textarea>
